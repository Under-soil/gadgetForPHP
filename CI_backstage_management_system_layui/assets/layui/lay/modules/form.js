            ...
            u = function() {
                this.config = {
                    verify: {
                        ...
                        username:[/^[0-9a-zA-Z\u4e00-\u9fa5][0-9a-zA-Z_@.\u4e00-\u9fa5]{0,18}[0-9a-zA-Z\u4e00-\u9fa5]$/,"用户格式不正确"],
                        appuid:[/^\d{8}$/,"游戏用户ID为8位数字"],
                        passwd:[/(?!^\[0-9]+$)(?!^[a-zA-Z]+$)(?!^[_#@]+$).{6,20}/,"密码必须6到20位且不能出现空格"],
                        realname:[/^([\u4e00-\u9fa5]+|([a-zA-Z]+\s?)+)$/,"请填写正确真实姓名"],
                        code:[/^\d{5}$/,"邀请码为5位数字"],
                        pcode:[/^\d{5}$/,"上级邀请码为5位数字"],
                        gcode:[/^\d{5}$/,"返佣者邀请码为5位数字"],
                        area_code:[/^\d{4}$/,"区域ID为4位数字"],
                        captcha:[/^\d{4}$/,"验证码格式不正确"],
                        wechat:[/^[a-zA-Z1-9]{1}[-_a-zA-Z0-9]{5,19}$/,"微信号格式不正确"],
                        qq:[/^\d*$/,"QQ格式不正确"],
                    }
                }
            };
